  ...
  describe("Testing Emergency functionality, Create a piggy, auction, buy, emergency settle", function() {
      ...
      it("Should emergency settle a piggy", function() {
        ...
        .then(result => {
          ...
          assert.strictEqual(result[6].logs[0].event, "ProposalRequest", "Event logs did not return correct event name")
          assert.strictEqual(result[6].logs[0].args.from, owner, "Event log from transfer didn't return correct sender")
          assert.strictEqual(result[6].logs[0].args.tokenId.toString(), tokenId.toString(), "Event log from transfer didn't return correct tokenId")
          assert.strictEqual(result[6].logs[0].args.proposalAmount.toString(), proposalAmount.toString(), "Event log from transfer didn't return correct tokenId")
        ...
        .then(result => {
          ...
          /* proposal request event */
          assert.strictEqual(result.logs[0].event, "ProposalRequest", "Event logs did not return correct event name")
          ...
          assert.strictEqual(result.logs[0].args.proposalAmount.toString(), proposalAmount.toString(), "Event log from transfer didn't return correct tokenId")
          /* settle piggy event */
          assert.strictEqual(result.logs[1].event, "SettlePiggy", "Event logs did not return correct event name")
          assert.strictEqual(result.logs[1].args.from, user01, "Event log from transfer didn't return correct sender")
          assert.strictEqual(result.logs[1].args.tokenId.toString(), tokenId.toString(), "Event log from transfer didn't return correct tokenId")
          assert.strictEqual(result.logs[1].args.holderPayout.toString(), proposalAmount.toString(), "Event log from transfer didn't return correct tokenId")
          assert.strictEqual(result.logs[1].args.writerPayout.toString(), proposalAmount.toString(), "Event log from transfer didn't return correct tokenId")
          return sequentialPromise([
      ...
      it("Should emergency settle after cross is made", function() {
        ...
        .then(result => {
          ...
          assert.strictEqual(result[6].logs[0].args.from, owner, "Event log from transfer didn't return correct sender")
          assert.strictEqual(result[6].logs[0].args.tokenId.toString(), tokenId.toString(), "Event log from transfer didn't return correct tokenId")
          assert.strictEqual(result[6].logs[0].args.proposalAmount.toString(), proposalAmount01.toString(), "Event log from transfer didn't return correct tokenId")
        ...
        .then(result => {
          ...
          /* cross not met no settle event should happen */
          assert.lengthOf(result.logs, 1, "Event logs did not return correct length")
        ...
        .then(result => {
          /* proposal fail */
          assert.lengthOf(result[0].logs, 1, "Event logs did not return correct length")
          assert.lengthOf(result[1].logs, 1, "Event logs did not return correct length")
        ...
        .then(result => {
          /* save a divsior as a big number */
          numberTwo = web3.utils.toBN(2)
          /* proposal passes with cross
          ** first proposal
          */
          assert.strictEqual(result[0].logs[0].event, "ProposalRequest", "Event logs did not return correct event name")
          assert.strictEqual(result[0].logs[0].args.from, owner, "Event log from transfer didn't return correct sender")
          assert.strictEqual(result[0].logs[0].args.tokenId.toString(), tokenId.toString(), "Event log from transfer didn't return correct tokenId")
          assert.strictEqual(result[0].logs[0].args.proposalAmount.toString(), proposalAmount03.toString(), "Event log from transfer didn't return correct tokenId")

          /* second proposal */
          assert.strictEqual(result[1].logs[0].event, "ProposalRequest", "Event logs did not return correct event name")
          ...
          assert.strictEqual(result[1].logs[0].args.proposalAmount.toString(), proposalAmount02.toString(), "Event log from transfer didn't return correct tokenId")

          assert.strictEqual(result[1].logs[1].event, "SettlePiggy", "Event logs did not return correct event name")
          assert.strictEqual(result[1].logs[1].args.from, user01, "Event log from transfer didn't return correct sender")
          assert.strictEqual(result[1].logs[1].args.tokenId.toString(), tokenId.toString(), "Event log from transfer didn't return correct tokenId")
          assert.strictEqual(result[1].logs[1].args.holderPayout.toString(), proposalAmount03.add(proposalAmount02).div(numberTwo).toString(), "Event log from transfer didn't return correct tokenId")
          assert.strictEqual(result[1].logs[1].args.writerPayout.toString(), collateral.sub(proposalAmount03.add(proposalAmount02).div(numberTwo)).toString(), "Event log from transfer didn't return correct tokenId")
      ...
      it("Should emergency settle a piggy that has received an oracle price", function() {
        ...
        .then(result => {
          ...
          assert.strictEqual(result[1].logs[1].args.from, user01, "Event log from transfer didn't return correct sender")
          assert.strictEqual(result[1].logs[1].args.tokenId.toString(), tokenId.toString(), "Event log from transfer didn't return correct tokenId")
          assert.strictEqual(result[1].logs[1].args.holderPayout.toString(), proposalAmount.toString(), "Event log from transfer didn't return correct tokenId")
          assert.strictEqual(result[1].logs[1].args.writerPayout.toString(), proposalAmount.toString(), "Event log from transfer didn't return correct tokenId")
          return sequentialPromise([
            () => Promise.resolve(piggyInstance.claimPayout(tokenInstance.address, proposalAmount, {from: owner})),
            () => Promise.resolve(piggyInstance.claimPayout(tokenInstance.address, proposalAmount, {from: user01})),
            () => Promise.resolve(tokenInstance.balanceOf(owner, {from: owner})),
      ...
      it("Should fail to emegency settle if non-holder/creator calls", function() {
        collateralERC = tokenInstance.address
        dataResolver = resolverInstance.address
        collateral = web3.utils.toBN(100 * decimals)
        lotSize = 10
        ...
        .then(result => {
          ...
          return expectedExceptionPromise(
            () => piggyInstance.proposeHolderShare(
              tokenId,
              proposalAmount,
      ...
      it("Should fail to emegency settle if piggy isn't expired", function() {
        collateralERC = tokenInstance.address
        dataResolver = resolverInstance.address
        collateral = web3.utils.toBN(100 * decimals)
        lotSize = 10
        strikePrice = 28000
        expiry = 100 // increase expiry
        isEuro = false
        isPut = true
        isRequest = false
        ...
        .then(result => {
          ...
          /* transaction should revert piggy not expired */
          return expectedExceptionPromise(
            () => piggyInstance.proposeHolderShare(
              tokenId,
              proposalAmount,
      ...
      it("Should fail to emegency settle if proposal is greater than collateral", function() {
        collateralERC = tokenInstance.address
        dataResolver = resolverInstance.address
        collateral = web3.utils.toBN(100 * decimals)
        lotSize = 10
